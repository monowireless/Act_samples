...
void print_pal(pktparser& pkt) {
	...
	if (pal.is_PalEvent()) {
		...
		case E_PAL_PCB::AMB:
		{
			...
			Serial << "PAL_AMB";
			Serial << ":TEMP=" << div100(amb.i16Temp) << 'C';
			Serial << ":HUMD=" << div100(amb.u16Humd) << '%';
...
void main_loop() {
	...
	while ((c = Serial.read()) >= 0) {
		...
		if (parser_ser.available()) {
			...
			switch (typ) {
			case E_PKT::PKT_PAL: pkt.parse<TwePacketPal>(payl.begin(), payl.end()); break;
			case E_PKT::PKT_ACT_STD: pkt.parse<TwePacketActStd>(payl.begin(), payl.end()); break;
			case E_PKT::PKT_TWELITE: pkt.parse<TwePacketTwelite>(payl.begin(), payl.end()); break;
			case E_PKT::PKT_APPIO: pkt.parse<TwePacketAppIO>(payl.begin(), payl.end()); break;
			case E_PKT::PKT_APPUART: pkt.parse<TwePacketAppUART>(payl.begin(), payl.end()); break;
			case E_PKT::PKT_APPTAG: pkt.parse<TwePacketAppTAG>(payl.begin(), payl.end()); break;
			default:
